#!/usr/bin/env bash
# Build the Portfolio AI Analyst example.
#
...
#
# UI half: the React + Vite WebView bundle -> ui/dist -> ui.zip (Vite builds
# src/main.tsx with base "./" and no module preload / inline scripts, so the
# bundle satisfies the host CSP: script-src 'self', loopback origin).
#
